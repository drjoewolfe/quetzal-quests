...
public class ArrayQuests {
			...
			for (int j = i + 1; j < n - 1; j++) {
				for (int k = i + 2; k < n; k++) {
					if (arr[i] + arr[j] + arr[k] == 0) {
						System.out.println("Triplets with zero sum found at indexes - " + i + ", " + j + ", " + k
								+ " : with values - " + arr[i] + ", " + arr[j] + ", " + arr[k]);
					}
				}
			}
		}
	...
	public static void findTripletsWithZeroSumHashing(int[] arr) {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			for (int j = i + 1; j < n; j++) {
				...
				if (map.contains(key)) {
					System.out.println(
							"Triplets with zero sum found with values - " + arr[i] + ", " + key + ", " + arr[j]);
				} else {
					map.add(arr[j]);
				}
			}
	...
	public static void findTripletsWithZeroSumSorting(int[] arr) {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			while (left < right) {
				...
				if (sum == 0) {
					System.out.println("Triplets with zero sum found at indexes - " + i + ", " + left + ", " + right
							+ " : with values - " + arr[i] + ", " + arr[left] + ", " + arr[right]);
					left++;
					right--;
				} else if (sum < 0) {
					left++;
	...
	private static int medianOfSortedArraysOfSameSizeDC(int[] arr1, int[] arr2, int arr1Start, int arr1End,
		...
		if (n == 2) {
			return (Math.max(arr1[arr1Start], arr2[arr2Start]) + Math.min(arr1[arr1Start + 1], arr2[arr2Start + 1]))
					/ 2;
		}
		...
		if (m1 == m2) {
			...
			if (n % 2 == 0)
				return medianOfSortedArraysOfSameSizeDC(arr1, arr2, arr1Start, arr1Start + n / 2 + 1,
						arr2Start + n / 2 - 1, arr2End);
			else
				return medianOfSortedArraysOfSameSizeDC(arr1, arr2, arr1Start, arr1Start + n / 2 + 1, arr2Start + n / 2,
						arr2End);
		} else {
			if (n % 2 == 0)
				return medianOfSortedArraysOfSameSizeDC(arr1, arr2, arr1Start + n / 2 + 1, arr1End, arr2Start,
						arr2Start - n / 2 + 1);
			else
				return medianOfSortedArraysOfSameSizeDC(arr1, arr2, arr1Start + n / 2, arr1End, arr2Start,
						arr2Start + n / 2 + 1);
		}
	}
	...
	public static int minimumDistanceBetweenTwoNumbers(int[] arr, int x, int y) {
		...
		for (; i < arr.length; i++) {
			if (arr[i] == x || arr[i] == y) {
				if (arr[prev] != arr[i] && (i - prev) < distance) {
					distance = i - prev;
					prev = i;
				} else {
					prev = i;
		...
		for (int i = 0; i < arr.length - 3; i++) {
			for (int j = i + 1; j < arr.length - 2; j++) {
				...
				while (left < right) {
					...
					if (currentSum == sum) {
						Quadruplet<Integer, Integer, Integer, Integer> result = new Quadruplet<>(arr[i], arr[j],
								arr[left], arr[right]);
						return result;
	...
	public static Quadruplet<Integer, Integer, Integer, Integer> findQuadrupletForSumA2(int[] arr, int sum) {
		...
		// Each triplet is of the form (i, j, sum); There will be n * (n-1) / 2 elements
		// in aux.
		...
		while (left < auxSize && right >= 0) {
			...
			if (leftSum.getFirst() == rightSum.getFirst() || leftSum.getSecond() == rightSum.getFirst()
					|| leftSum.getFirst() == rightSum.getSecond() || leftSum.getSecond() == rightSum.getSecond()) {
				commonElementsFound = true;
			}

			if (currentSum == sum && !commonElementsFound) {
				return new Quadruplet<>(arr[leftSum.getFirst()], arr[leftSum.getSecond()], arr[rightSum.getFirst()],
	...
	public static void convertToZigZag(int[] arr) {
		...
		for (int i = 0; i < arr.length - 1; i++) {
			if (greaterThanFlag && arr[i] < arr[i + 1]) {
				Utilities.swap(arr, i, i + 1);
			} else if (!greaterThanFlag && arr[i] > arr[i + 1]) {
				Utilities.swap(arr, i, i + 1);
			}
	...
	public static int lengthOfSmallestSubarrayWithGreaterSum(int[] arr, int sum) {
		...
		while (end < arr.length) {
			while (runningSum <= sum && end < arr.length) {
				runningSum += arr[end++];
			}

			while (runningSum > sum && start < arr.length) {
	...
	public static int lengthOfSmallestSubarrayWithGreaterSumNaive(int[] arr, int sum) {
		...
		for (int i = 0; i < arr.length; i++) {
			...
			for (int j = i; j < arr.length; j++) {
				...
				if (runningSum > sum && minLength > length) {
					minLength = length;
	...
	public static int getSmallestMissingNumberInSortedArray(int[] arr) {
		...
		// Approach 2: Linerar traversal of array, checking a[i] with a[i+1]. If
		// difference is not 1, it is the missing number
		// Approach 3: Modified binary search, with comparison of index & values. First
		// location where arr[i] != i gives the missing number
	...
	public static Triplet<Integer, Integer, Integer> getIncreasingSubsequenceOfLengthThreeWithMaxProductApproach2(
			int[] arr) {
		if (arr == null || arr.length < 3) {
			return null;
		}
		...
		for (int i = 1; i < arr.length - 1; i++) {
			...
			for (int j = 0; j < i; j++) {
				if (arr[j] < b && arr[j] > largestElementToLeftSmallerThanB) {
			...
			for (int j = i + 1; j < arr.length; j++) {
				if (b < arr[j] && arr[j] > largestElementToRightGreaterThanB) {
	...
	public static Triplet<Integer, Integer, Integer> getIncreasingSubsequenceOfLengthThreeWithMaxProductNaive(
			int[] arr) {
		if (arr == null || arr.length < 3) {
			return null;
		}
	...
	public static int[] subArrayOfPositiveIntegersWithGivenSum(int[] arr, int sum) {
		...
		for (int i = 1; i < arr.length; i++) {
			while (runningSum > sum && start < i - 1) {
				runningSum -= arr[start++];
			}
	...
	public static int getMaxArraySumAfterKNegations(int[] arr, int k) {
		// For each of the k iterations, find the lowest number in the array & negate
		// it.
	...
	public static int getMaxSumOfIncreasingOrderSingleElementsFromNArrays(int[][] arrays) {
		// Start from the last array, and find the largest elements still valid in all
		// arrays for the sum
		...
		for (int[] arr : arrays) {
			if (arr == null || arr.length != numElements) {
				return Integer.MIN_VALUE;
			}
		}

		int sum = 0;
		int lastLargest = Integer.MAX_VALUE;
		for (int i = numArrays - 1; i >= 0; i--) {
			int[] arr = arrays[i];

			int largest = Integer.MIN_VALUE;
			for (int j = 0; j < numElements; j++) {
				if (largest < arr[j] && arr[j] < lastLargest) {
					largest = arr[j];
				}
			}

			if (largest == Integer.MIN_VALUE) {
				// All elements of the current array are greater than the previous largest
